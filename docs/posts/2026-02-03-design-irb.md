...
- The `Report` object is just a queue of checks which are not executed until the user calls `.show()` (pretty report for notebook) or `.run()` (for manual inspection of the tables/charts). 
