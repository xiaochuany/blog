...
The hazard function says how likely it is for the event to happen at time $t$ given that it has not happened before $t$. This translates in the discrete case to

$$
h(t) = P[T=t|T\ge t]
$$

or in the continuous case to

$$
h(t) = \lim_{dt\to 0} \frac{P[T\in (t,t+dt]|T\ge t]}{dt}
$$
...
The independence between $T$ and $C$ implies 
...
where $H_T$ is the cumulative hazard function of $T$ and $F_{Y,1}(u)=P[Y\le u, \Delta=1]$. To see this, we have

$$
P[T\in dt] = P[Y\in dt, \Delta=1] + P[T\in dt, C<t ]  
$$

from which we derive $P[T\le dt] = P[Y\in dt, \Delta=1]/P[C\ge t]$. Also,  

$$
P[T\ge t]=P[Y\ge t] + P[T\ge t, C<t ]  
$$

from which we derive $P[T\ge t] = P[Y\ge t]/P[C\ge t]$. Applying these in the definiton of hazard for $T$ proves the desired claim (see note[^1] for details) 
...
To see this, observe that the empirical counterparty of $F_{Y,1}$ is $\frac{1}{n}\sum_i  \Delta_i \delta_{Y_i}$. The empirical version of $S_Y(s-)$ is $\frac{1}{n}\sharp\{j: Y_j \ge s\}$. As a result, the empirical counterpart of the measure $F_{Y,1}(ds)/S_Y(s-)$ is 

$$\sum_i  \frac{\Delta_i\delta_{Y_i}(du)}{\sharp\{j: Y_j\ge u\}}.$$ 

For each fixed $i$, the summand measure is a  Dirac measure on $Y_i$ scaled by a counting factor if $\Delta_i=1$, zero otherwise. The claim follows immediately. 
