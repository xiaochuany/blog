...
`jax.numpy` module is straightforward to use for `numpy` users, which has a near identical API. Like other parts of `jax`, these operations are built on top of the XLA compiler intermediate representations for high performance numerical computations. 
