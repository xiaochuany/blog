...
More control over the operating axes of `vmap` is possible. Below  `in_axes=(0,None)` adds batch dimension at the leading axis for the  first argument of the function. Without specifying `in_axes`, the `vmap(f)` would vectorize all leave nodes of the input PyTree. In our example, the supplied inputs are 1D array with different size, therefore incompatible if we were to set `in_axis=0`.
...
y = jnp.array([4, 5])
out = vmap(f, in_axes=(0, None), out_axes=1)(x=xs, y=y)
print(out)
```
