...
In other words, vmap is like `np.stack` individual results of a function. Using `vmap` can avoid  manual batching, manual stacking etc.  
...
Additionally, we can specify `out_axes` along which we stack the results, to use again our `np.stack` analogy.    
...
out = jax.vmap(li, in_axes=(None,0), out_axes=1)(
    beta, 
...
Be aware that `jax` follows the functional programming paradigm. This implies explicit key handling for samplers. The samplers can be composed with `vmap` to achieve vectorization across all parameters. 
