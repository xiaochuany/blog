...
We can add social media links in the footer. 

We can also add in the background an analytics tool. The easiest solution is GA4 from google since we do not own a custom domain. 
In GA, enhanced GA measurement toggles are mostly optional, for blogs, only page views, scrolls, outbound clicks matter. 
